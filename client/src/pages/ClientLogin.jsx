...
import { AiOutlineWhatsApp, AiFillEye, AiFillEyeInvisible, AiFillCheckCircle } from 'react-icons/ai';
  ...
  const handleSubmit = async(e) => {
    ...
    if (isRegistered) {
      ...
      if (email && contactNumber && whatsappNumber && password) {
       await signUp({name, email, contactNumber, whatsappNumber, password}, navigate, dispatch);
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        {!isRegistered && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="whatsapp" className="block text-sm font-medium mb-1 flex items-center gap-2">
                <AiOutlineWhatsApp className="text-green-500" /> WhatsApp Number:
              </label>
              <input
                type="text"
                id="whatsapp"
        ...
        {!isRegistered && (
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="sameAsContact" className="text-sm">Same as Contact Number</label>
          </div>
        )}
        ...
        <div className={`mt-5 ${validation ? "flex" : "hidden"} flex-wrap gap-4`}>
          {ValidationData.map((item) => (
            <div className="flex gap-2 items-center" key={item.id}>
              ...
              <p className={`${item.regx.test(password) ? "text-green-500" : "text-red-500"}`}>{item.name}</p>
            </div>
          ))}
        </div>
